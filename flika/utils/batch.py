...
class BatchStep:
    ...
    def __repr__(self):
        ...
        return f'BatchStep({self.name}, {args})'
...
class BatchRunner:
    ...
    def run(self, file_paths: List[str], output_dir: Optional[str] = None,
        ...
        from ..process.file_ import open_file, save_file

        results = []
        total = len(file_paths)

        for idx, fpath in enumerate(file_paths):
            logger.info(f'Batch [{idx + 1}/{total}]: {os.path.basename(fpath)}')
            g.m.statusBar().showMessage(f'Batch [{idx + 1}/{total}]: {os.path.basename(fpath)}')

            try:
                open_file(fpath)
            except Exception as e:
                logger.error(f'Batch: failed to open {fpath}: {e}')
                results.append('')
                continue
            ...
            for step in self.steps:
                try:
                    ...
                    logger.error(f'Batch: step {step.name} failed on {fpath}: {e}')
                    break
            ...
            if output_dir is not None and g.win is not None:
                ...
                try:
                    ...
                    logger.error(f'Batch: failed to save {out_path}: {e}')
                    results.append('')
            else:
                results.append('')

        g.m.statusBar().showMessage(f'Batch complete: processed {total} files')
        return results

    def __repr__(self):
        step_str = '\n  '.join(repr(s) for s in self.steps)
        return f'BatchRunner(\n  {step_str}\n)'
