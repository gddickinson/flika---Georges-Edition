...
from flika.utils.BaseProcess import BaseProcess, WindowSelector, MissingWindowError
from flika.utils.custom_widgets import CheckBox, ComboBox, SliderLabel
...
class SPT_Analysis(BaseProcess):
    ...
    def __call__(self, keepSourceWindow=True):
        ...
        panel.show()
        panel.raise_()
        if g.m is not None: g.m.statusBar().showMessage('SPT Analysis panel opened.')
...
class Detect_Particles(BaseProcess):
    ...
    def __call__(self, method='U-Track', psf_sigma=1.5, alpha=0.05,
        ...
        if g.m is not None: g.m.statusBar().showMessage(f'Detecting particles ({method})
        ...
        if g.m is not None: g.m.statusBar().showMessage(
...
class Link_Particles_Process(BaseProcess):
    ...
    def __call__(self, method='Greedy', max_distance=5, max_gap=1,
        ...
        if g.m is not None: g.m.statusBar().showMessage(f'Linking particles ({method})
        ...
        from flika.spt.linking.greedy_linker import tracks_to_dict
    ...
    def _display_tracks(self, window, tracks_dict):
        ...
        try:
            from flika.viewers.track_overlay import TrackOverlay, show_track_overlay
