...
# ---------------------------------------------------------------------------
# PSF generation helpers (self-contained)
# ---------------------------------------------------------------------------
...
def _gaussian_psf(size, sigma):
    ...
    ----------
    size : int
        Side length of the kernel (should be odd).
    sigma : float
        Standard deviation.

    Returns
    -------
    ndarray (size, size), normalized to sum to 1.
    """
...
def _airy_psf(size, radius):
    """Generate an approximate Airy disk PSF.
    ...
    size : int
        Side length of the kernel (should be odd).
    radius : float
        First-zero radius of the Airy pattern.
...
def _apply_to_stack(func, tif, *args):
    """Apply a 2D function to each frame of a 2D or 3D array."""
    if tif.ndim == 2:
        return func(tif, *args)
    elif tif.ndim == 3:
        result = np.zeros_like(tif, dtype=np.float64)
        for i in range(tif.shape[0]):
            result[i] = func(tif[i], *args)
        return result
    return tif
...
class Richardson_Lucy(BaseProcess):
    ...
    def __call__(self, psf_sigma, psf_size=11, iterations=20, keepSourceWindow=False):
        ...
        self.newtif = _apply_to_stack(
            _richardson_lucy_impl, self.tif.astype(np.float64), psf, int(iterations))
...
class Wiener_Deconvolution(BaseProcess):
    ...
    def __call__(self, psf_sigma, psf_size=11, noise_variance=0.01, keepSourceWindow=False):
        ...
        psf = _gaussian_psf(int(psf_size), psf_sigma)
        self.newtif = _apply_to_stack(
            _wiener_impl, self.tif.astype(np.float64), psf, noise_variance)
