...
class Time_Stamp(BaseProcess):
    ...
    def gui(self):
        ...
        elif 'framerate' in g.settings.d.keys() and g.settings['framerate'] is not None:
...
class Scale_Bar(BaseProcess):
    ...
    def gui(self):
        ...
        width_pixels.setRange(1, int(w.mx))
    ...
    def __call__(self,width_microns, width_pixels, font_size, color, background,location,show=True,keepSourceWindow=None):
        ...
        if show:
            ...
            if location=='Top Left':
                barPoint=QtCore.QPointF(0, textRect.height())
            elif location=='Top Right':
                barPoint=QtCore.QPointF(-width_pixels, textRect.height())
            elif location=='Lower Right':
                barPoint=QtCore.QPointF(-width_pixels, -textRect.height())
            elif location=='Lower Left':
                barPoint=QtCore.QPointF(0, -textRect.height())
    ...
    def updateBar(self):
        ...
        if location=='Top Left':
            barPoint=QtCore.QPointF(0, 1.3*textHeight)
            w.scaleBarLabel.setPos(QtCore.QPointF(width_pixels/2-textWidth/2,0))
        elif location=='Top Right':
            barPoint=QtCore.QPointF(w.mx-width_pixels, 1.3*textHeight)
            w.scaleBarLabel.setPos(QtCore.QPointF(w.mx-width_pixels/2-textWidth/2,0))
        elif location=='Lower Right':
            barPoint=QtCore.QPointF(w.mx-width_pixels, w.my-1.3*textHeight)
            w.scaleBarLabel.setPos(QtCore.QPointF(w.mx-width_pixels/2-textWidth/2,w.my-textHeight))
        elif location=='Lower Left':
            barPoint=QtCore.QPointF(0, w.my-1.3*textHeight)
            w.scaleBarLabel.setPos(QtCore.QPointF(width_pixels/2-textWidth/2,w.my-textHeight))
