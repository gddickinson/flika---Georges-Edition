# -*- coding: utf-8 -*-
"""Batch and video export processes.

Provides video export (H.264/MJPEG) and batch export of all open windows
as TIFF, PNG, or NumPy files.
"""
import numpy as np
import os
...
from flika.utils.BaseProcess import BaseProcess_noPriorWindow, BaseProcess
from flika.utils.custom_widgets import WindowSelector, SliderLabel, CheckBox, ComboBox
...
class Video_Exporter(BaseProcess_noPriorWindow):
    ...
    def export_video(self):
        ...
        if g.m is not None:
            g.m.statusBar().showMessage('Exporting video...')
        ...
        if g.m is not None:
            g.m.statusBar().showMessage('Video exported successfully to {}'.format(filepath))
...
class Batch_Export(BaseProcess_noPriorWindow):
    ...
    def _do_export(self):
        ...
        for win in windows:
            ...
            if as_seq and img.ndim >= 3 and ext != '.npy':
                ...
                for t in range(img.shape[0]):
                    frame_path = os.path.join(seq_dir, 'frame_{:05d}{}'.format(t, ext))
        ...
        if g.m is not None:
            g.m.statusBar().showMessage('Exported {} files to {}'.format(exported, out_dir))
        self.ui.accept()
