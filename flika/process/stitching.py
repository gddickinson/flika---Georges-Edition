...
from flika.utils.custom_widgets import WindowSelector, SliderLabel, ComboBox, CheckBox, MissingWindowError
...
class Stitch_Images(BaseProcess):
    ...
    def __call__(self, window1, window2, direction='Horizontal', overlap=0.1,
        ...
        if g.m is not None:
            g.m.statusBar().showMessage('Stitching images...')
        ...
        self.newname = '{} + {} (Stitched)'.format(window1.name, window2.name)

        if keepSourceWindow is False:
            window2.close()
        if g.m is not None:
            g.m.statusBar().showMessage('Stitching complete. Shift: dy={}, dx={}'.format(dy, dx))
        return self.end()
