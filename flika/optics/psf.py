# -*- coding: utf-8 -*-
"""Canonical Point Spread Function library for flika.

This module consolidates all PSF generation routines into a single
authoritative location.  Other modules (simulation, deconvolution, etc.)
should import from here rather than maintaining their own copies.

Models
------
- ``gaussian_psf_2d`` / ``gaussian_psf_3d`` -- Gaussian approximations
- ``airy_psf_2d`` -- Physical Airy disk pattern
- ``born_wolf_psf_3d`` -- Scalar Born-Wolf diffraction model
- ``vectorial_psf_3d`` -- Richards-Wolf vectorial model
- ``astigmatic_psf_3d`` -- z-dependent astigmatic Gaussian (for 3D SMLM)
- ``generate_psf`` -- Factory dispatching to any model by name

Convenience wrappers (simple size+sigma interface):
- ``gaussian_psf(size, sigma)``
- ``airy_psf(size, radius)``
...
def normalize_psf(psf):
    """Normalize a PSF so that it sums to 1.

    Parameters
    ----------
    psf : ndarray
        Input PSF of any dimensionality.

    Returns
    -------
    ndarray
        Copy of *psf* scaled so that ``psf.sum() == 1``.  If the input
        sums to zero the array is returned unchanged (as a copy).
    """
...
# ---------------------------------------------------------------------------
# 2D Gaussian PSF (physics-parameterised)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2D Airy disk PSF (physical parameters)
# ---------------------------------------------------------------------------
...
def airy_psf_2d(shape, wavelength, NA, pixel_size):
    ...
    wl_um = wavelength / 1000.0  # nm to um
    k = 2 * np.pi * NA / wl_um
    v = k * r
...
# ---------------------------------------------------------------------------
# 3D Born-Wolf scalar diffraction PSF
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3D Vectorial (Richards-Wolf) PSF
# ---------------------------------------------------------------------------
...
def astigmatic_psf_3d(shape, sigma_x_func, sigma_y_func, z_positions):
    ...
    sigma_x_func : callable
        Function mapping z (um) to sigma_x (pixels).
    sigma_y_func : callable
        Function mapping z (um) to sigma_y (pixels).
...
# ---------------------------------------------------------------------------
# Convenience wrappers (simple size + sigma/radius interface)
# ---------------------------------------------------------------------------

def gaussian_psf(size, sigma):
    """Generate a normalized 2D Gaussian PSF with a simple interface.

    This is the convenience wrapper used by deconvolution routines.
    For the full-featured version with arbitrary shape and center,
    use :func:`gaussian_psf_2d`.

    Parameters
    ----------
    size : int
        Side length of the square output array.
    sigma : float
    ...
    -------
    ndarray
        Normalized (size, size) Gaussian PSF.
    """
...
def airy_psf(size, radius):
    """Generate a normalized 2D Airy disk PSF approximation.

    This is the convenience wrapper used by deconvolution routines.
    For the physically parameterised version (wavelength, NA, pixel size),
    use :func:`airy_psf_2d`.
    ...
    size : int
        Side length of the square output array.
    radius : float
        Radius parameter controlling the Airy disk scale (in pixels).

    Returns
    -------
    ndarray
        Normalized (size, size) Airy PSF.
    """
    x = np.arange(size) - size // 2
    y = x[:, None]
    x = x[None, :]
