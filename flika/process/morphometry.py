...
Provides region properties (area, perimeter, circularity, eccentricity,
etc.), Haralick texture features, and Hu moment shape descriptors.
"""
...
def compute_shape_descriptors(mask):
    ...
    if mask.sum() == 0:
        return {'hu_{}'.format(i + 1): np.nan for i in range(7)}
...
class MorphometryAnalysis(BaseProcess):
    ...
    def __call__(self, window, threshold=0.5, include_texture=False,
        ...
        print("=" * 60)
        print("Morphometry Analysis: {} ({} regions)".format(name, n_regions))
        print("=" * 60)
        for r in regions[:10]:  # Print first 10
            print("  Region {}: area={}, circ={:.3f}, ecc={:.3f}, aspect={:.2f}".format(
                r['label'], r['area'],
                r.get('circularity', 0), r.get('eccentricity', 0),
                r.get('aspect_ratio', 0)))
        if n_regions > 10:
            print("  ... and {} more regions".format(n_regions - 10))
        print("=" * 60)

        # Summary stats
        areas = [r['area'] for r in regions]
        print("  Total regions      = {}".format(n_regions))
        print("  Mean area          = {:.1f}".format(np.mean(areas)))
        print("  Median area        = {:.1f}".format(np.median(areas)))
        print("  Total area         = {}".format(np.sum(areas)))
        print("=" * 60)
        ...
        if g.m is not None:
            g.m.statusBar().showMessage(
                "Morphometry: {} regions, mean area={:.1f}".format(
                    n_regions, np.mean(areas)))
