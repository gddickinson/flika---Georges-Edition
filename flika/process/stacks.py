...
class Generate_Random_Image(BaseProcess_noPriorWindow):
    ...
    def gui(self):
        ...
        depth = SliderLabel(0)
        depth.setRange(2,256)
        self.items.append({'name':'dimensions','string':'Dimensions','object':dimensions})
        self.items.append({'name':'nFrames','string':'Number of Frames (T)','object':nFrames})
        self.items.append({'name':'width','string':'Width (X)','object':width})
        self.items.append({'name':'height','string':'Height (Y)','object':height})
        self.items.append({'name':'depth','string':'Depth (Z) or Channels (C)','object':depth})
        super().gui()

    def __call__(self, nFrames=10000, width=128, height=128, depth=10, dimensions='3D (T, X, Y)'):
        self.start()
...
def _make_cylinder(vol, cx, cy, cz, r, half_h, axis, value):
    ...
    bbox = [
        (max(0, int(cx - r)), min(vol.shape[0], int(cx + r + 1))),
        (max(0, int(cy - r)), min(vol.shape[1], int(cy + r + 1))),
        (max(0, int(cz - r)), min(vol.shape[2], int(cz + r + 1))),
    ]
    # Extend along the cylinder axis
    centres = [cx, cy, cz]
    halfs = [r, r, r]
    halfs[axis] = half_h
    for i in range(3):
        bbox[i] = (max(0, int(centres[i] - halfs[i])),
    ...
    xx, yy, zz = np.meshgrid(xs, ys, zs, indexing='ij')
    coords = [xx, yy, zz]
    # Radial axes are the two that aren't the cylinder axis
...
class Generate_Phantom_Volume(BaseProcess_noPriorWindow):
    ...
    def __call__(self, nFrames=10, width=64, height=64, depth=32, nObjects=12,
        ...
        # Pre-generate object parameters
        ...
        if content_type == 'Moving blobs':
            # Each object has a position that drifts over time
            obj_params = []
            ...
            # Static objects (same in every frame, with noise varying)
        ...
        self.newtif = vol
        self.newname = f'Phantom {content_type} {width}x{height}x{depth}'
        return self.end()
...
@per_plane
def _resize_impl(tif, factor):
    import skimage.transform
    if tif.ndim == 3:
        mt, mx, my = tif.shape
        result = np.zeros((mt, mx * factor, my * factor))
        for t in range(mt):
            result[t] = skimage.transform.resize(tif[t], (mx * factor, my * factor))
        return result
    elif tif.ndim == 2:
        mx, my = tif.shape
        return skimage.transform.resize(tif, (mx * factor, my * factor))
    return tif
...
class Resize(BaseProcess):
    ...
    def __call__(self,factor,keepSourceWindow=False):
        ...
        if self.tif.dtype in (np.uint64, np.int64):
            ...
            return
        is_rgb = self.oldwindow.metadata.get('is_rgb', False)
        if not is_rgb:
            self.newtif = _resize_impl(self.tif, factor)
        else:
            self.newtif = None
...
class ZProject(BaseProcess):
    ...
    def gui(self):
        ...
        if g.win and len(g.win.image.shape) < 3:
            g.alert('zproject requires at least 3 dimensional windows')
            return False
        if g.win is not None:
            nFrames=g.win.image.shape[0]
        firstFrame=QtWidgets.QSpinBox()
...
def _get_shear_coordinates(image_2d, theta):
    ...
    Returns (old_coords, new_coords) where each is [row_indices, col_indices].
    """
    mx, my = image_2d.shape
    S = _get_shear_transform_matrix(theta)
    S_inv = np.linalg.inv(S)

    # Transform the four corners to find output bounds
...
class Shear_Transform(BaseProcess):
    ...
    imaging geometry.  Operates on 3-D or 4-D data.

    Parameters:
        theta (float): Shear angle in degrees (typically 0-90).
        shift_factor (int): Up-sampling factor along the shear axis before
            transforming. Higher values reduce information loss.
        interpolate (bool): If True, use smooth rescaling for up-sampling
            instead of pixel repetition.
    Returns:
        newWindow
    """
    ...
    def __call__(self, theta=45.0, shift_factor=1, interpolate=False, keepSourceWindow=False):
        ...
        if tif.ndim == 3:
            # (T, X, Y) — shear along X-Y plane per frame
            result = self._shear_stack(tif, theta, shift_factor, interpolate, dtype)
        elif tif.ndim == 4:
            # (T, X, Y, Z) — shear X vs Z for each time point
        ...
        self.newtif = result
        self.newname = self.oldname + f' - Sheared {theta}°'
        return self.end()

    @staticmethod
    def _upsample_axis(arr, axis, factor, interpolate):
        """Up-sample *arr* along *axis* by *factor*."""
        if factor <= 1:
            return arr
        if interpolate:
            try:
                from skimage.transform import rescale
                # Build scale tuple: 1.0 for all axes except target
                scales = [1.0] * arr.ndim
    ...
    def _shear_stack(self, tif, theta, shift_factor, interpolate, dtype):
        # Up-sample along axis 1 (X)
        tif = self._upsample_axis(tif, 1, shift_factor, interpolate)
        # Compute coordinate map from first frame
    ...
    def _shear_4d(self, tif, theta, shift_factor, interpolate, dtype):
        # tif is (T, X, Y, Z).  Shear in the X-Z plane for each Y slice.
        # Reorder to (T, Z, Y, X) so we can shear Z vs X
        A = np.moveaxis(tif, 3, 1)  # (T, Z, Y, X)
        mt, mz, my, mx = A.shape
        # Up-sample Z
        A = self._upsample_axis(A, 1, shift_factor, interpolate)
        mz_up = A.shape[1]
        # Compute coord map from a (Z, X) slice
        ...
        use_parallel = g.settings.get('multiprocessing', True) and mt > 4
        if use_parallel:
        ...
        out = np.moveaxis(out, 1, 3)  # (T, nc, my, nr) → move axis 1 to 3
        return out
...
class Motion_Correction(BaseProcess):
    """motion_correction(reference_frame, keepSourceWindow=False)

    Corrects translational drift using phase cross-correlation.
    Each frame/volume is aligned to the reference frame.

    Parameters:
        reference_frame (int): Index of the reference frame to align to.
    Returns:
        newWindow
    """
    ...
    def gui(self):
        ...
        if g.win is not None:
            nFrames = g.win.image.shape[0]
        ref.setRange(0, max(0, nFrames - 1))
        ref.setValue(0)
        self.items.append({'name': 'reference_frame', 'string': 'Reference Frame', 'object': ref})
        super().gui()

    def __call__(self, reference_frame=0, keepSourceWindow=False):
        self.start(keepSourceWindow)
        tif = self.tif
        if tif.ndim < 3:
            g.alert('Motion correction requires at least 3D data (T, X, Y)')
            return None

        from skimage.registration import phase_cross_correlation
        from scipy.ndimage import shift as ndi_shift

        corrected = np.zeros_like(tif)
        ref_idx = min(reference_frame, tif.shape[0] - 1)
        reference = tif[ref_idx]

        def correct_frame(i):
            if i == ref_idx:
                return tif[i]
            detected_shift, _, _ = phase_cross_correlation(reference, tif[i])
            return ndi_shift(tif[i], detected_shift, mode='constant', cval=0)
        ...
        if use_parallel:
            ...
            n_workers = g.settings.get('nCores', 4)
            with ThreadPoolExecutor(max_workers=n_workers) as executor:
                results = list(executor.map(correct_frame, range(tif.shape[0])))
        ...
        self.newname = self.oldname + ' - Motion Corrected'
        return self.end()
