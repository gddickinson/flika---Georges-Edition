...
from skimage import filters, morphology, transform, feature, draw as skdraw
...
# ---------------------------------------------------------------------------
# Inlined drawing helpers (from flika.utils.drawing, not in this repo)
# ---------------------------------------------------------------------------

def _draw_line(image, y0, x0, y1, x1, value=1.0):
...
def _draw_circle(image, center_y, center_x, radius, value=1.0):
    """Draw a circle perimeter on a 2D image."""
    rr, cc = skdraw.circle_perimeter(int(round(center_y)), int(round(center_x)),
                                     int(round(radius)), shape=image.shape)
    image[rr, cc] = value


def _draw_crosses(image, centers_yx, size=3, value=1.0):
        ...
        if tif.ndim == 3:
            ...
            for i in range(len(tif)):
                ...
                if analysis['branch_points']:
                    _draw_crosses(marked, analysis['branch_points'], size=2, value=mark_val)
                if analysis['endpoints']:
                    _draw_crosses(marked, analysis['endpoints'], size=1, value=mark_val * 0.7)
                result[i] = marked
        elif tif.ndim == 2:
    ...
    def __call__(self, threshold=10, line_length=50, line_gap=10, keepSourceWindow=False):
        ...
        def _detect_frame(frame_2d, frame_idx):
            ...
            for (x0, y0), (x1, y1) in lines:
                _draw_line(marked, y0, x0, y1, x1, mark_val)
    ...
    def __call__(self, min_radius=10, max_radius=50, num_peaks=10, min_distance=20, keepSourceWindow=False):
        ...
        def _detect_frame(frame_2d, frame_idx):
            ...
            for acc, x, y, r in zip(accums, cx, cy, found_radii):
                _draw_circle(marked, y, x, r, mark_val)
                all_circles.append({
                    'frame': frame_idx,
                    'center_y': int(y),
                    'center_x': int(x),
...
class Corner_Detection(BaseProcess):
    ...
    def __call__(self, method='Harris', min_distance=5, sigma=1.0, keepSourceWindow=False):
        ...
        def _detect_frame(frame_2d, frame_idx):
            ...
            marked = frame_2d.copy()
            mark_val = max(np.max(marked) * 1.2, 1.0)
            _draw_crosses(marked, coords, size=2, value=mark_val)
            for c in coords:
