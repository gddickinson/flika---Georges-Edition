...
from flika.utils.custom_widgets import ComboBox
...
class Sobel_Filter(BaseProcess):
    ...
    def gui(self) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "preview", "string": "Preview", "object": preview}
        )
        super().gui()
        self.preview()
...
def _laplacian_impl(tif, ksize):
    if tif.ndim == 2:
        ...
        result = np.zeros(tif.shape)
        for i in range(len(tif)):
            result[i] = skimage.filters.laplace(
                tif[i].astype(np.float64), ksize=ksize
            )
        return result
...
class Laplacian_Filter(BaseProcess):
    ...
    def gui(self) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "ksize", "string": "Kernel Size", "object": ksize}
        )
        self.items.append(
            {"name": "preview", "string": "Preview", "object": preview}
        )
    ...
    def __call__(self, ksize: int = 3, keepSourceWindow: bool = False):
        self.start(keepSourceWindow)
...
class Gaussian_Laplace_Filter(BaseProcess):
    ...
    def __call__(self, sigma: float = 1.0, keepSourceWindow: bool = False):
        self.start(keepSourceWindow)
...
def _gaussian_gradient_magnitude_impl(tif, sigma):
    if tif.ndim == 2:
        ...
        result = np.zeros(tif.shape)
        for i in range(len(tif)):
            result[i] = gaussian_gradient_magnitude(
                tif[i].astype(np.float64), sigma=sigma
            )
        return result


class Gaussian_Gradient_Magnitude_Filter(BaseProcess):
    ...
    def __call__(self, sigma: float = 1.0, keepSourceWindow: bool = False):
        ...
        self.newtif = self.newtif.astype(g.settings["internal_data_type"])
        self.newname = (
            self.oldname + " - Gaussian Gradient Magnitude sigma=" + str(sigma)
        )
        return self.end()
...
gaussian_gradient_magnitude_filter = Gaussian_Gradient_Magnitude_Filter()
...
def _sato_impl(tif, sigma_min, sigma_max, black_ridges):
    ...
    if tif.ndim == 2:
        return skimage.filters.sato(
            tif.astype(np.float64), sigmas=sigmas, black_ridges=black_ridges
        )
    elif tif.ndim == 3:
        result = np.zeros(tif.shape)
        for i in range(len(tif)):
            result[i] = skimage.filters.sato(
                tif[i].astype(np.float64), sigmas=sigmas, black_ridges=black_ridges
            )
        return result
...
class Sato_Tubeness(BaseProcess):
    ...
    def gui(self) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "sigma_min", "string": "Sigma Min", "object": sigma_min}
        )
        self.items.append(
            {"name": "sigma_max", "string": "Sigma Max", "object": sigma_max}
        )
        self.items.append(
            {"name": "black_ridges", "string": "Black Ridges", "object": black_ridges}
        )
        self.items.append(
            {"name": "preview", "string": "Preview", "object": preview}
        )
    ...
    def preview(self) -> None:
        ...
        if preview:
            if len(g.win.image.shape) == 3:
                ...
                testimage = g.win.image.astype(np.float64)
            sigmas = np.linspace(
                sigma_min, sigma_max, max(int(sigma_max - sigma_min) + 1, 2)
            )
            testimage = skimage.filters.sato(
                testimage, sigmas=sigmas, black_ridges=black_ridges
            )
            g.win.imageview.setImage(testimage, autoLevels=False)
        else:
            g.win.reset()
...
def _meijering_impl(tif, sigma_min, sigma_max, black_ridges):
    ...
    if tif.ndim == 2:
        ...
        result = np.zeros(tif.shape)
        for i in range(len(tif)):
            result[i] = skimage.filters.meijering(
                tif[i].astype(np.float64), sigmas=sigmas, black_ridges=black_ridges
            )
        return result
...
class Meijering_Neuriteness(BaseProcess):
    ...
    def preview(self) -> None:
        ...
        if preview:
            ...
            testimage = skimage.filters.meijering(
                testimage, sigmas=sigmas, black_ridges=black_ridges
            )
            g.win.imageview.setImage(testimage, autoLevels=False)
        else:
...
def _hessian_impl(tif, sigma_min, sigma_max, black_ridges):
    sigmas = np.linspace(sigma_min, sigma_max, max(int(sigma_max - sigma_min) + 1, 2))
    if tif.ndim == 2:
        return skimage.filters.hessian(
            tif.astype(np.float64), sigmas=sigmas, black_ridges=black_ridges
        )
    elif tif.ndim == 3:
        result = np.zeros(tif.shape)
        for i in range(len(tif)):
            result[i] = skimage.filters.hessian(
                tif[i].astype(np.float64), sigmas=sigmas, black_ridges=black_ridges
            )
        return result
...
class Hessian_Filter(BaseProcess):
    ...
    def preview(self) -> None:
        ...
        if preview:
            ...
            testimage = skimage.filters.hessian(
                testimage, sigmas=sigmas, black_ridges=black_ridges
            )
            g.win.imageview.setImage(testimage, autoLevels=False)
        else:
...
def _gabor_impl(tif, frequency, theta):
    if tif.ndim == 2:
        real, _ = skimage.filters.gabor(
            tif.astype(np.float64), frequency=frequency, theta=theta
        )
        return real
    elif tif.ndim == 3:
        result = np.zeros(tif.shape)
        for i in range(len(tif)):
            real, _ = skimage.filters.gabor(
                tif[i].astype(np.float64), frequency=frequency, theta=theta
            )
            result[i] = real
        return result
...
class Gabor_Filter(BaseProcess):
    ...
    def gui(self) -> None:
        ...
        self.items.append(
            {
                "name": "frequency",
                "string": "Frequency (cycles/pixel)",
                "object": frequency,
            }
        )
        self.items.append(
            {"name": "theta", "string": "Theta (degrees)", "object": theta}
        )
        self.items.append(
            {"name": "preview", "string": "Preview", "object": preview}
        )
    ...
    def __call__(
        ...
        self.newtif = self.newtif.astype(g.settings["internal_data_type"])
        self.newname = self.oldname + " - Gabor freq={} theta={}".format(
            frequency, theta
        )
        return self.end()
    ...
    def preview(self) -> None:
        ...
        if preview:
            if len(g.win.image.shape) == 3:
                ...
                testimage = g.win.image.astype(np.float64)
            theta_rad = np.deg2rad(theta)
            testimage, _ = skimage.filters.gabor(
                testimage, frequency=frequency, theta=theta_rad
            )
            g.win.imageview.setImage(testimage, autoLevels=False)
        else:
            g.win.reset()
...
def _percentile_filter_impl(tif, percentile, size):
    if tif.ndim == 2:
        return nd_percentile_filter(
            tif.astype(np.float64), percentile=percentile, size=size
        )
    elif tif.ndim == 3:
        result = np.zeros(tif.shape)
        for i in range(len(tif)):
            result[i] = nd_percentile_filter(
                tif[i].astype(np.float64), percentile=percentile, size=size
            )
        return result
...
class Maximum_Filter(BaseProcess):
    ...
    def gui(self) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "size", "string": "Kernel Size", "object": size}
        )
        self.items.append(
            {"name": "preview", "string": "Preview", "object": preview}
        )
    ...
    def __call__(self, size: int = 3, keepSourceWindow: bool = False):
        self.start(keepSourceWindow)
        self.newtif = _maximum_filter_impl(self.tif, size)
        self.newtif = self.newtif.astype(g.settings["internal_data_type"])
        self.newname = self.oldname + " - Maximum Filter size={}".format(size)
        return self.end()
...
class Minimum_Filter(BaseProcess):
    ...
    def __call__(self, size: int = 3, keepSourceWindow: bool = False):
        ...
        self.newtif = self.newtif.astype(g.settings["internal_data_type"])
        self.newname = self.oldname + " - Minimum Filter size={}".format(size)
        return self.end()
...
class Percentile_Filter(BaseProcess):
    ...
    def gui(self) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "percentile", "string": "Percentile", "object": percentile}
        )
        self.items.append(
            {"name": "size", "string": "Kernel Size", "object": size}
        )
    ...
    def __call__(
        ...
        self.newtif = self.newtif.astype(g.settings["internal_data_type"])
        self.newname = self.oldname + " - Percentile Filter p={} size={}".format(
            percentile, size
        )
        return self.end()
    ...
    def preview(self) -> None:
        ...
        if preview:
            if len(g.win.image.shape) == 3:
                ...
                testimage = g.win.image.astype(np.float64)
            testimage = nd_percentile_filter(
                testimage, percentile=percentile, size=size
            )
            g.win.imageview.setImage(testimage, autoLevels=False)
        else:
            g.win.reset()
...
def _tv_denoise_impl(tif, weight):
    if tif.ndim == 2:
        return skimage.restoration.denoise_tv_chambolle(
            tif.astype(np.float64), weight=weight
        )
    elif tif.ndim == 3:
        result = np.zeros(tif.shape)
        for i in range(len(tif)):
            result[i] = skimage.restoration.denoise_tv_chambolle(
                tif[i].astype(np.float64), weight=weight
            )
        return result
...
class TV_Denoise(BaseProcess):
    ...
    Args:
        weight (float): Denoising weight. Higher values remove more noise but also
            more detail.
    ...
    def gui(self) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "weight", "string": "Weight", "object": weight}
        )
        self.items.append(
            {"name": "preview", "string": "Preview", "object": preview}
        )
    ...
    def __call__(self, weight: float = 0.1, keepSourceWindow: bool = False):
        self.start(keepSourceWindow)
    ...
    def preview(self) -> None:
        ...
        if preview:
            if len(g.win.image.shape) == 3:
                ...
                testimage = g.win.image.astype(np.float64)
            testimage = skimage.restoration.denoise_tv_chambolle(
                testimage, weight=weight
            )
            g.win.imageview.setImage(testimage, autoLevels=False)
        else:
            g.win.reset()
...
class Flash_Remover(BaseProcess):
    ...
    def _auto_detect_flash(
        self, tif: np.ndarray, window_size: int
    ) -> tuple[int, int]:
    ...
    def _linear_interpolation(
        self, tif: np.ndarray, flash_start: int, flash_end: int
    ) -> np.ndarray:
        """Replace flash frames by linearly interpolating between pre/post flash."""
        ...
        for idx, i in enumerate(range(flash_start, flash_end + 1)):
            ...
            result[i] = (
                np.interp([alpha], [0, 1], [0, 1])[0] * result[post_frame]
                + (1 - alpha) * result[pre_frame]
            )
        return result

    def _noise_scaling(
        self, tif: np.ndarray, flash_start: int, flash_end: int, window_size: int
    ) -> np.ndarray:
        ...
        if pre_end > pre_start and post_end > post_start:
            baseline = np.concatenate(
                [result[pre_start:pre_end], result[post_start:post_end]], axis=0
            )
    ...
    def gui(self) -> None:
        ...
        self.items.append(
            {"name": "flash_start", "string": "Flash Start Frame", "object": flash_start}
        )
        self.items.append(
            {"name": "flash_end", "string": "Flash End Frame", "object": flash_end}
        )
        self.items.append(
            {"name": "auto_detect", "string": "Auto Detect", "object": auto_detect}
        )
        self.items.append(
            {"name": "window_size", "string": "Window Size", "object": window_size}
        )
        super().gui()
    ...
    def __call__(
        ...
        if method == "Linear Interpolation":
            ...
            self.newtif = self._noise_scaling(
                self.tif, flash_start, flash_end, window_size
            )
        else:
            self.newtif = self.tif
        self.newtif = self.newtif.astype(g.settings["internal_data_type"])
...
class Bleach_Correction(BaseProcess):
    ...
    Parameters:
        method (str): Correction method.
            ...
            'Ratio to Mean' -- divides each frame by its mean, then multiplies by
            the stack mean.
    Returns:
        newWindow
    """
    ...
    def __call__(
        self, method: str = "Exponential Fit", keepSourceWindow: bool = False
    ):
        self.start(keepSourceWindow)
        if self.tif.ndim < 3:
    ...
    def _exponential_fit(self, tif: np.ndarray) -> np.ndarray:
        from scipy.optimize import curve_fit

        def _exp_decay(t, amplitude, rate, offset):
            return amplitude * np.exp(-rate * t) + offset
        ...
        try:
            ...
            popt, _ = curve_fit(_exp_decay, t_vals, mean_trace, p0=p0, maxfev=5000)
            fit = _exp_decay(t_vals, *popt)
    ...
    def _histogram_match(self, tif: np.ndarray) -> np.ndarray:
    ...
    def _ratio_to_mean(self, tif: np.ndarray) -> np.ndarray:
        mt = tif.shape[0]
