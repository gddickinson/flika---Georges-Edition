# -*- coding: utf-8 -*-
"""Spectral unmixing of multi-channel images.

Provides linear spectral unmixing (NNLS/least-squares), PCA-based
endmember estimation, and residual computation.
"""
...
class SpectralUnmixing(BaseProcess):
    ...
    def __call__(self, window, n_components=3, method='nnls',
        ...
        if img.ndim == 3:
            ...
            g.alert("Unsupported image dimensions: {}".format(img.ndim))
            return
        ...
        print("=" * 55)
        print("Spectral Unmixing: {}".format(name))
        print("=" * 55)
        print("  Channels           = {}".format(n_channels))
        print("  Components         = {}".format(n_comp))
        print("  Method             = {}".format(method))
        print("  RMSE               = {:.6f}".format(rmse))
        for i in range(n_comp):
            ab = abundances[..., i]
            print("  Component {}: mean={:.4f}, max={:.4f}".format(
                i, np.mean(ab), np.max(ab)))
        print("=" * 55)
        ...
        if g.m is not None:
            g.m.statusBar().showMessage(
                "Spectral unmixing: {} components, RMSE={:.4f}".format(n_comp, rmse))
        ...
        ab_window = Window(abundance_stack,
                           name='Unmixed: {} ({} components)'.format(name, n_comp))

        # Plot endmember spectra
        self._spectra_widget = QtWidgets.QWidget()
        self._spectra_widget.setWindowTitle('Endmember Spectra: {}'.format(name))
