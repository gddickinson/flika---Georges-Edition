...
Provides apparent and corrected FRET efficiency computation, stoichiometry
ratio, histogram analysis, and summary statistics.
"""
...
from flika.utils.custom_widgets import WindowSelector, SliderLabel, CheckBox, MissingWindowError
    ...
    def __call__(self, donor_window, acceptor_window, bleedthrough=0.0,
        ...
        if donor_2d.shape != acceptor_2d.shape:
            g.alert("Donor and acceptor images must have the same dimensions. "
                    "Got {} and {}.".format(donor_2d.shape, acceptor_2d.shape))
            return
        ...
        print("=" * 55)
        print("FRET Analysis: {} / {}".format(name_d, name_a))
        print("=" * 55)
        print("  Method             = {}".format('corrected' if corrected else 'apparent'))
        if corrected:
            print("  Bleedthrough       = {:.4f}".format(bleedthrough))
            print("  Direct excitation  = {:.4f}".format(direct_excitation))
            print("  Gamma              = {:.4f}".format(gamma))
        print("  Mean FRET E        = {:.4f}".format(stats['mean_E']))
        print("  Median FRET E      = {:.4f}".format(stats['median_E']))
        print("  Std FRET E         = {:.4f}".format(stats['std_E']))
        print("  N pixels           = {}".format(stats['n_pixels']))
        print("  Mean Stoichiometry = {:.4f}".format(stoich_stats['mean_E']))
        print("=" * 55)
        ...
        if g.m is not None:
            g.m.statusBar().showMessage(
                "FRET: E={:.3f} +/- {:.3f}, n={}".format(
                    stats['mean_E'], stats['std_E'], stats['n_pixels']))

        # Create FRET efficiency image window
        from flika.window import Window
        fret_window = Window(E, name='FRET E: {}/{}'.format(name_d, name_a))

        # Create histogram
        self._hist_widget = QtWidgets.QWidget()
        self._hist_widget.setWindowTitle('FRET Histogram: {}/{}'.format(name_d, name_a))
        ...
        if np.isfinite(stats['mean_E']):
            mean_line = pg.InfiniteLine(
                ...
                label="mean={:.3f}".format(stats['mean_E']),
