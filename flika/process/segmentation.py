# -*- coding: utf-8 -*-
"""Segmentation processes: connected components, region properties,
SLIC superpixels, random walker, boundaries, and contours.
"""
...
from flika.utils.custom_widgets import SliderLabel, CheckBox, ComboBox, WindowSelector
...
class Connected_Components(BaseProcess):
    ...
    def __call__(self, connectivity=1, keepSourceWindow=False):
        ...
        if w is not None:
            ...
            if g.m is not None:
                g.m.statusBar().showMessage('Found {} connected components'.format(count))
        return w
...
class Region_Properties(BaseProcess):
    ...
    def __call__(self, min_area=1, max_area=0, intensity_window=None, keepSourceWindow=False):
        ...
        if w is not None:
            ...
            n = len(region_list)
            if g.m is not None:
                g.m.statusBar().showMessage('Measured {} regions'.format(n))
        return w
...
class Find_Contours_Process(BaseProcess):
    ...
    def __call__(self, level=0.5, keepSourceWindow=False):
        ...
        n = len(contours)
        if g.m is not None:
            g.m.statusBar().showMessage('Found {} contours, created ROIs'.format(n))
        return self.end()
