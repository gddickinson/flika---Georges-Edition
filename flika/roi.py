...
class ROI_Drawing(pg.GraphicsObject):
    ...
    def paint(self, p, *args):
        ...
        elif self.kind == 'ellipse' or self.kind == 'center_surround':
    ...
    def drawFinished(self):
        ...
        elif self.kind == 'center_surround':
            pts = [self.state['pos'], self.state['size']]
        else:
            pts = self.pts
...
class ROI_Base():
    ...
    Attributes:
        name: display name for this ROI (auto-generated if not provided)
    ...
    def getMask(self):
        ...
        Results are cached and invalidated when the ROI moves.
        '''
        if not self._mask_dirty and self._mask_cache is not None:
            return self._mask_cache
        result = self._compute_mask()
        self._mask_cache = result
        self._mask_dirty = False
        return result

    def _compute_mask(self):
        '''Compute the mask — subclasses override this instead of getMask.'''
    ...
    def plot(self):
        ...
        # Redraw trace when a Z/C slider changes the displayed slice (4D)
        if hasattr(self.window, 'sigSliceChanged'):
            self.window.sigSliceChanged.connect(self.redraw_trace)
    ...
    def unplot(self):
        ...
        if hasattr(self.window, 'sigSliceChanged'):
            try:
                self.window.sigSliceChanged.disconnect(self.redraw_trace)
            except (TypeError, RuntimeError):
                pass
        if self.traceWindow is not None:
    ...
    def _show_measure(self):
        ...
        from .utils.roi_stats import compute_roi_stats, compute_shape_descriptors
        stats = compute_roi_stats(self, self.window)
        shape = compute_shape_descriptors(self, self.window)
        lines = [f"ROI: {getattr(self, 'name', self.kind)}",
                 f"Type: {self.kind}", ""]
    ...
    def _show_histogram(self):
        """Show live histogram of pixel values within this ROI."""
        from .viewers.roi_histogram import ROIHistogramWidget
        self._histogram_widget = ROIHistogramWidget(self)
    ...
    def makeMenu(self):
        ...
        # Transform submenu
        transformMenu = QtWidgets.QMenu("Transform", self.menu)
        transformMenu.addAction("Enlarge...", lambda: self._transform_mask('enlarge'))
        transformMenu.addAction("Shrink...", lambda: self._transform_mask('shrink'))
        transformMenu.addAction("Make Band...", lambda: self._transform_mask('band'))
        transformMenu.addAction("Convex Hull", lambda: self._transform_mask('convex_hull'))

        def updateMenu():
            ndims = self.window.nDims
            is_rgb = self.window.metadata.get('is_rgb', False)
            has_time = ndims >= 3 and not is_rgb
            plotAct.setVisible(has_time or self.window.volume is not None)
        ...
        self.menu.addMenu(transformMenu)
        self.menu.aboutToShow.connect(updateMenu)

    def _transform_mask(self, operation):
        """Apply a morphological transform to this ROI mask, creating a new freehand ROI."""
        pixels = 1
        if operation in ('enlarge', 'shrink', 'band'):
            pixels, ok = QtWidgets.QInputDialog.getInt(
                None, f"{operation.title()} ROI", "Pixels:", value=3, min=1, max=50)
            if not ok:
                return
        new_roi = transform_roi_mask(self, operation, pixels)
        if new_roi is not None:
            g.m.statusBar().showMessage(f"Created {operation} ROI")
    ...
    def delete(self):
        ...
        if hasattr(self.window, 'sigROIRemoved'):
            self.window.sigROIRemoved.emit(self)
    ...
    def drawFinished(self):
        ...
        self.window.currentROI = self
        if hasattr(self.window, 'sigROICreated'):
            self.window.sigROICreated.emit(self)
...
class ROI_line(ROI_Base, pg.LineSegmentROI):
    ...
    def _show_line_profile(self):
        """Show intensity profile along this line ROI."""
        from .viewers.line_profile import LineProfileWidget
        self._line_profile_widget = LineProfileWidget(self)
        self._line_profile_widget.show()

    def makeMenu(self):
        ROI_Base.makeMenu(self)
        lineProfileAct = QtWidgets.QAction("&Line Profile", self, triggered=self._show_line_profile)
        self.menu.addAction(lineProfileAct)
...
class ROI_ellipse(ROI_Base, pg.EllipseROI):
    ...
    def _compute_mask(self):
        ...
        rr, cc = ellipse(center_r, center_c, r_radius, c_radius,
...
class ROI_point(ROI_Base, pg.ROI):
    ...
    def getTrace(self, bounds=None):
        ...
        s1, s2 = self.getMask()
        if np.size(s1) == 0:
            return np.zeros(self.window.mt)
...
class ROI_center_surround(ROI_Base, pg.EllipseROI):
    ...
    def __init__(self, window, pts, inner_ratio=None, shape_mode=None, **kargs):
        ...
        self.inner_ratio = inner_ratio if inner_ratio is not None else (g.settings.get('cs_inner_ratio', 0.5) or 0.5)
    ...
    def _draw_shape(self, p, rect, dashed=False):
        ...
        if dashed:
            ...
            # Compensate for the scale transform applied in paint()
    ...
    def paint(self, p, *args):
        r = self.boundingRect()
        p.setRenderHint(QtGui.QPainter.Antialiasing)
        # Scale to unit rect (same approach as EllipseROI.paint)
    ...
    def _get_region_mask(self, center_r, center_c, r_radius, c_radius):
        ...
        if self.shape_mode == 'square':
            ...
            xx, yy = np.meshgrid(np.arange(xmin, xmax, dtype=int),
                                 np.arange(ymin, ymax, dtype=int))
            return xx.flatten(), yy.flatten()
        else:
            from skimage.draw import ellipse
            return ellipse(center_r, center_c, r_radius, c_radius, shape=shape)
    ...
    def getCenterMask(self):
        """Inner region mask."""
        pos = np.array(self.state['pos'], dtype=float)
        size = np.array(self.state['size'], dtype=float)
        cr = pos[0] + size[0] / 2.0
        cc = pos[1] + size[1] / 2.0
        return self._get_region_mask(cr, cc,
                                     abs(size[0]) / 2.0 * self.inner_ratio,
                                     abs(size[1]) / 2.0 * self.inner_ratio)

    def getSurroundMask(self):
    ...
    def makeMenu(self):
        ...
        def plotCenter():
            from .tracefig import roiPlot
            # Temporarily swap getMask to center-only
            orig = self._compute_mask
            self._compute_mask = lambda: self.getCenterMask()
            self._invalidate_mask_cache()
            self.traceWindow = roiPlot(self)
            self._compute_mask = orig
            self._invalidate_mask_cache()

        def plotSurround():
            from .tracefig import roiPlot
            orig = self._compute_mask
            self._compute_mask = lambda: self.getSurroundMask()
            self._invalidate_mask_cache()
            self.traceWindow = roiPlot(self)
            self._compute_mask = orig
            self._invalidate_mask_cache()
        ...
        def adjustSettings():
            ...
            dlg.setWindowTitle("Center-Surround Settings")
            layout = QtWidgets.QFormLayout(dlg)

            ratio_spin = QtWidgets.QDoubleSpinBox()
            ratio_spin.setRange(0.05, 0.95)
            ratio_spin.setSingleStep(0.05)
            ratio_spin.setDecimals(2)
            ratio_spin.setValue(self.inner_ratio)
            layout.addRow("Inner / Outer ratio:", ratio_spin)

            shape_combo = QtWidgets.QComboBox()
            for s in ['circle', 'ellipse', 'square']:
                shape_combo.addItem(s)
            shape_combo.setCurrentIndex(shape_combo.findText(self.shape_mode))
            ...
            if dlg.exec_():
                self.inner_ratio = ratio_spin.value()
                new_shape = shape_combo.currentText()
                if new_shape != self.shape_mode:
                    self.shape_mode = new_shape
                    if self.shape_mode in ('circle', 'square'):
                        size = np.array(self.state['size'], dtype=float)
                        side = max(abs(size[0]), abs(size[1]))
                        self.setSize([side, side], finish=False)
                self._invalidate_mask_cache()
                self.update()
                if self.traceWindow is not None:
                    self.traceWindow.translateFinished(self)
        ...
        self.menu.addAction("Settings...", adjustSettings)
...
class ROI_rect_line(ROI_Base, QtWidgets.QGraphicsObject):
    ...
    def getHandlePositions(self):
        """Return the positions of all handles in parent coordinates.

        Computes from each line ROI's state rather than scene transforms,
        so this works correctly even before the ROI is added to a scene.
        """
        from pyqtgraph import Point as PgPoint
        def _handle_parent_pos(line, handle_frac):
            """Map a handle's fractional position through the line ROI's transform."""
            local = PgPoint(handle_frac[0], handle_frac[1]) * line.state['size']
            return line.mapToParent(local)

        p = _handle_parent_pos(self.lines[0], [0, 0.5])
        p = self.lines[0].getSnapPosition([p.x(), p.y()])
        pos = [p]
        for line in self.lines:
            p = _handle_parent_pos(line, [1, 0.5])
    ...
    def addSegment(self, pos=(0, 0), connectTo=None):
        ...
        if len(self.lines) == 0 or ind > 0: # add handles in order
            ## Add both SR handles before moving (pyqtgraph 0.14+ requires
            ## both handles to exist for ScaleRotate computation to work).
            ...
            ## Now move handles to desired positions
            if isinstance(connectTo, Handle):
                newRoi.movePoint(connectTo, connectTo.scenePos(), coords='scene')
            else:
                newRoi.movePoint(h, connectTo)
...
def makeROI_from_mask(mask, window):
...
def transform_roi_mask(roi, operation, pixels=1):
...
def boolean_roi_op(roi_a, roi_b, operation='AND'):
