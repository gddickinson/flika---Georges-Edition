...
class BaseProcess(object):
    ...
    def start(self, keepSourceWindow):
        ...
        if g.settings.get('apply_to_all_planes', False) and self.oldwindow.volume is not None:
            self.tif = self.oldwindow.volume
        else:
    ...
    def end(self):
        ...
        if not hasattr(self, 'newtif') or self.newtif is None:
            ...
            return
        # Capture input shape before the old window may be closed
        _input_shape = tuple(self.oldwindow.image.shape)
        # Save state for undo before creating the new window
        from ..core.undo import undo_stack, ProcessCommand
        cmd = ProcessCommand(
            old_image=self.oldwindow.image.copy(),
            old_name=self.oldwindow.name,
            old_filename=self.oldwindow.filename,
            old_commands=self.oldwindow.commands[:],
            old_metadata=dict(self.oldwindow.metadata),
            new_window=None,  # will be set below
            command_str=getattr(self, 'command', self.__name__),
            keep_source=self.keepSourceWindow,
            old_volume=self.oldwindow.volume.copy() if self.oldwindow.volume is not None else None,
        )
        ...
        cmd.capture_new_state(newWindow)
        undo_stack.push(cmd)
        ...
        # Record structured provenance data
        import time as _time_mod
        if hasattr(self, '_start_time'):
            duration = _time_mod.time() - self._start_time
            from ..app.macro_recorder import macro_recorder
            macro_recorder.record_structured(
                command=getattr(self, 'command', self.__name__),
                duration_seconds=duration,
                input_window=self.oldname,
