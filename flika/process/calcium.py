...
class CalciumAnalysis(BaseProcess):
    ...
    def __call__(self, window, baseline_frames=10, baseline_method='mean',
        ...
        print("=" * 55)
        print(f"Calcium Analysis: {name}")
        print("=" * 55)
        print(f"  Baseline frames    = {baseline_frames}")
        print(f"  Baseline method    = {baseline_method}")
        print(f"  Detection thresh   = {threshold}")
        print(f"  Events detected    = {stats['n_events']}")
        print(f"  Mean amplitude     = {stats['mean_amplitude']:.4f}")
        print(f"  Mean duration      = {stats['mean_duration']:.1f} frames")
        print(f"  Mean rise time     = {stats['mean_rise_time']:.1f} frames")
        print(f"  Mean decay time    = {stats['mean_decay_time']:.1f} frames")
        print(f"  Mean area          = {stats['mean_area']:.4f}")
        print(f"  Frequency          = {stats['frequency']:.4f} events/frame")
        print("=" * 55)

        window.metadata['calcium'] = results

        g.status_msg(
            f"Calcium: {stats['n_events']} events, "
            f"amp={stats['mean_amplitude']:.3f}, "
            f"dur={stats['mean_duration']:.1f}f"
        )
        ...
        self._trace_widget.setWindowTitle(f'Calcium Trace: {name}')
        ...
        pw.setTitle(f'Calcium Trace: {name}')
        layout.addWidget(pw)
        ...
        stats_text = (
            ...
            f"Mean dur: {stats['mean_duration']:.1f}f"
        )
