...
class ColocalizationScatterWidget(QtWidgets.QWidget):
    ...
    def __init__(self, ch1_data, ch2_data, name1, name2, mask=None, parent=None):
        ...
        self.setWindowTitle(f'Colocalization: {name1} vs {name2}')
        self.resize(550, 500)
...
class Colocalization(BaseProcess):
    ...
    def __call__(self, window1, window2, use_roi=False, keepSourceWindow=True):
        ...
        print("=" * 50)
        print(f"Colocalization Analysis: {name1} vs {name2}")
        print("=" * 50)
        print(f"  Pearson r          = {r:.4f}")
        print(f"  Costes thresh ch1  = {t1:.4f}")
        print(f"  Costes thresh ch2  = {t2:.4f}")
        print(f"  Costes r (above)   = {r_above:.4f}")
        print(f"  Costes r (below)   = {r_below:.4f}")
        print(f"  Manders M1         = {M1:.4f}")
        print(f"  Manders M2         = {M2:.4f}")
        print(f"  Li ICQ             = {icq:.4f}")
        print("=" * 50)
